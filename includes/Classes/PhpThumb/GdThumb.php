...
class GdThumb  extends ThumbBase
{
    ...
    public function blurImage(int $pow = 5){

        $dimensions = $this->getCurrentDimensions();

        $smallestWidth = ceil($dimensions['width'] * pow(0.5, $pow));
        $smallestHeight = ceil($dimensions['height'] * pow(0.5, $pow));

        // For the first run, the previous image is the original input.
        $prevImage = $this->oldImage;
        ...
        imagecopyresized(
            $this->oldImage, $nextImage,
            0, 0, 0, 0,
            $dimensions['width'], $dimensions['height'], $nextWidth, $nextHeight
        );
     ...
     * @return GdThumb
     */
    public function save ($fileName, $saveOriginal = true,  $format = null)
    {
        ...
        imagewebp($this->oldImage, $webpName);
